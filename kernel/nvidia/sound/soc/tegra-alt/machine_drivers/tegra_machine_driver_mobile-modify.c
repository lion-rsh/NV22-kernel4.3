				  ...
				  u64 formats)
{
	...
	if (rtd) {
		...
		err = snd_soc_dai_set_sysclk(rtd->codec_dai, 0,
					     12000000, SND_SOC_CLOCK_IN);//modify RT5659_SCLK_S_MCLK->0;aud_mclk->12000000
		if (err < 0) {
	...
	/*added by lk: link.name = record-i2s1 */
...
static int codec_init(struct tegra_machine *machine)
{
	...
	for (i = 0; i < num_links; i++) {
		...
		if (strstr(dai_links[i].name, "playback-i2s2") ||
		    strstr(dai_links[i].name, "record-i2s1"))
